...
const ABI = [
  "function register(address _upline) external", "function buyNFT() external", "function claimExpiredNFT(uint256 nftId) external",
  "function users(address) view returns (bool isReg, address upline, uint256 directs, uint256 earned, uint256 invToday, uint256 lastInvTime, uint256 nfsId)",
  ...
  "function getMarketQueue() view returns (uint256[])", "function basePrice() view returns (uint256)",
  "function claimTimer() view returns (uint256)", "function userNftHistory(address, uint256) view returns (uint256)"
];
...
export default function App() {
  ...
  const [user, setUser] = useState({ isReg: false, earned: "0.0", directs: 0, nfsId: 0, inv: "0.0", lastInvTime: 0 });
  ...
  const fetchData = async () => {
    try {
      ...
      setUser({ isReg: u[0], earned: ethers.formatEther(u[3]), directs: Number(u[2]), nfsId: Number(u[6]), inv: ethers.formatEther(u[4]), lastInvTime: Number(u[5]) });
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-white font-sans selection:bg-cyan-500/30 pb-20">
      <div className="max-w-md mx-auto relative pt-4 px-4">
        ...
        {tab === 'Income' && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="bg-[#111827] border border-cyan-500/30 rounded-3xl p-8 mb-6 shadow-[0_0_30px_rgba(6,182,212,0.1)] text-center relative overflow-hidden flex flex-col items-center justify-center h-64">
               <div className="absolute w-48 h-48 rounded-full border-[6px] border-b-transparent border-l-cyan-500 border-t-blue-500 border-r-purple-500 opacity-60 animate-[spin_10s_linear_infinite]"></div>
               <div className="relative z-10 bg-[#0a0f1a] w-32 h-32 rounded-full flex flex-col items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.8)] border border-gray-800">
                  <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">Total</p>
                  <h2 className="text-2xl font-black text-white">{user.earned}</h2>
